...
Phiên bản native 0.1 nhận diện topology theo type/port, không phụ thuộc id, vị trí hoặc chiều hiển thị:
...
AWGN có thể lấy SNR từ Experiment hoặc giá trị fixed của block. Graph có thêm sink quan sát, Python Block, codec khác hoặc topology khác sẽ chạy compatibility engine. Chọn `Native` thay vì `Auto` để yêu cầu nghiêm ngặt; graph không hỗ trợ sẽ báo lỗi rõ ràng.
