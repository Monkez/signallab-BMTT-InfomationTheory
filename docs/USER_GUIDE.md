...
- `Device = CPU`: native CPU hoặc NumPy tùy executor. `Auto/GPU` vẫn dành cho backend CuPy/CUDA tương thích; chúng không làm một graph bất kỳ tự động chạy trên GPU.
...
- **Run Benchmark** là nút chạy thí nghiệm theo mode hiện tại. Trong lúc chạy, nút **Results** có hiệu ứng ánh sáng và bong bóng một dòng nhỏ ngay bên dưới hiển thị phần trăm, step và SNR/frame hiện tại. Khi hoàn tất nút chuyển xanh, bong bóng tự biến mất; mở Results bằng nút xanh. Port preview đại diện được lấy từ một frame xác định tại SNR đầu tiên; dữ liệu đầy đủ của mọi frame không được gửi lên UI nên app vẫn nhẹ với mô phỏng lớn.
...
The Console automatically follows newly appended messages while you are at the bottom. Scroll upward to inspect history without being pulled back down; returning near the bottom re-enables following. During a benchmark, the benchmark toolbar button changes to a red Stop icon. The Results dialog also provides **Stop benchmark**; cancellation is cooperative and the job ends with a cancelled status after the current bounded batch exits.

Numeric configuration fields use transactional editing. You can erase the entire value, leave the field temporarily empty, and type decimal, grouped integer or scientific notation (`2.345.100`, `1e6`, `1.52e3`). Grouped thousands separators are shown automatically after commit. The value is applied only when you press **Enter**; **Escape** or leaving the field with an unconfirmed edit restores the previous value.

Inspector and sink Details use the same numeric editor, including Spectrum Analyzer's Details action, scope/constellation axes, FFT floor/range, metrics, and BER point tables. This keeps editing consistent across result views and avoids accidental clamping while a value is being replaced.
