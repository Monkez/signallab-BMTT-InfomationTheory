...
- Kết quả hiển thị tên executor thực tế như `native_cpp`, `python_numpy` hoặc `python_multiprocessing`. Native result ghi modulation/coding plan; compatibility result giải thích lý do Auto fallback, ví dụ graph có thêm sink waveform. Có thể chạy `benchmark.bat` để so native với Python một worker hoặc `benchmark_regression.bat` để kiểm tra BPSK/QPSK/16-QAM. Xem chi tiết tại [Native CPU Engine](NATIVE_ENGINE.md).
