...
- Kết quả hiển thị tên executor thực tế như `native_cpp`, `python_numpy` hoặc `python_multiprocessing`. Có thể chạy `benchmark.bat` để so native với Python một worker trên cùng graph và seed. Xem chi tiết tại [Native CPU Engine](NATIVE_ENGINE.md).
