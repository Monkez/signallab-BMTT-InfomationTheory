        ...
        <section className="experiment-config-modal config-only" role="dialog" aria-modal="true" aria-labelledby="experiment-config-title">
          ...
          <div className="experiment-config-body config-only">
            <div className="experiment-config-form">
              ...
              <label>Execution engine<select disabled={executionActive} value={config.engine} onChange={e => setConfig({ ...config, engine: e.target.value as SimulationConfig['engine'] })}><option value="auto">Auto · native when supported</option><option value="native">Native · C++/oneTBB only</option><option value="python">Python · compatibility engine</option></select><small>Auto uses the fused native engine for supported benchmark graphs and falls back safely.</small></label>
              ...
              <label>Compute device<select disabled={executionActive} value={config.device} onChange={e => setConfig({ ...config, device: e.target.value as SimulationConfig['device'] })}><option value="auto">Auto · best available</option><option value="cpu">CPU · native or NumPy</option><option value="gpu">GPU · CUDA/CuPy</option></select></label>
      ...
      {resultsOpen && <div className="experiment-config-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setResultsOpen(false) }}>
        <section className="experiment-config-modal results-modal" role="dialog" aria-modal="true" aria-labelledby="results-title">
          ...
          <div className="results-modal-body">
            ...
            {(result || hasVisibleSinkResults) ? <div className="results" ref={resultsRef}>{result && <div className="metric-row results-runtime"><div className="metric"><span>Elapsed</span><strong>{result.elapsed_seconds.toFixed(2)} s</strong></div><div className="metric"><span>Throughput</span><strong>{formatNumber(result.throughput_bps / 1000)} kb/s</strong></div>{result.execution?.modulation && <div className="metric"><span>Native plan</span><strong>{result.execution.modulation.toUpperCase()} · {result.execution.coding}</strong></div>}</div>}{result?.execution?.fallback_reason && <p className="warning">Auto used the compatibility engine: {result.execution.fallback_reason}</p>}<SinkResults nodes={sinkNodes} metrics={activeSinkMetrics} berPoints={livePoints} berLive={job?.status === 'running'} />{result?.warnings?.map(w => <p className="warning" key={w}>{w}</p>)}</div> : <div className="experiment-config-empty"><Activity size={28} /><strong>No runtime results</strong><span>Run once or Run Benchmark from the floating toolbar.</span></div>}
          </div>
        </section>
      </div>}
