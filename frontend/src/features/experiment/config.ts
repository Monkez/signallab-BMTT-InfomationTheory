...
export function validateSimulationConfig(config: SimulationConfig) {
  ...
  if (!Number.isSafeInteger(config.max_frames) || config.max_frames < 1) return 'Max frames must be a positive integer (scientific notation is supported).'
  if (!Number.isSafeInteger(config.min_frames) || config.min_frames < 1) return 'Min frames must be a positive integer.'
  if (config.min_frames > config.max_frames) return 'Min frames cannot exceed max frames.'
  if (!Number.isSafeInteger(config.min_errors) || config.min_errors < 0) return 'Min errors must be a non-negative integer.'
